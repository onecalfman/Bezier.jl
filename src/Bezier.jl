...
bezier(x :: Vector{T}, range :: StepRangeLen = 0:0.01:1) where T <: Real = map(t -> bernstein((x |> length) - 1, t, x), range)

bezier(x :: Vector{T}, y :: Vector{S}, range :: StepRangeLen = 0:0.01:1) where {T,S <: Number} = (bezier(x, range), bezier(y, range))
